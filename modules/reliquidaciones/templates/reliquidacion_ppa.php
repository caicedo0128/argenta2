...
function calcularReliquidacionPPA(){
    ...
    if ($("#fecha_pago_pactada").val() != "" && $("#fecha_real_pago").val() != "")
    {
        ...
        var interesesDevolver = Math.round(valorObligacionPactada - (valorObligacionPactada / potencia));
        ...
        var valorObligacion = valorObligacionPactada - interesesDevolver - abono - diferenciaTotalIVA;
...
function calcularReliquidacionPPAbono(){
    ...
    var fechaPagoPactada = new Date($("#fecha_pago_pactada_abono").val());
    var fechaUltimoPago = new Date($("#fecha_ultimo_pago_abono").val());
    var fechaPago = new Date($("#fecha_real_pago_abono").val());
    var fechaPagoOperacion = new Date($("#fecha_real_pago_operacion").val());
    var tasaOperacion = new Date($("#tasa_operacion").val());
	...
	var diasValidacionIntereses = DateDiff.inDays(fechaPago, fechaUltimoPago);
    ...
    //INTERESES A DEVOLVER
    var interesesDevolver = Math.round((valorObligacion / potencia) - valorObligacion);
    var interesesSumarObligacion = 0;
    if (diasValidacionIntereses <= 0){
    	interesesSumarObligacion = interesesDevolver;
    	interesesDevolver = 0;    	
    }
    
    $("#intereses_devolver_abono").val(interesesDevolver);
    $("#text_intereses_devolver_abono").text(interesesDevolver);
    $('#text_intereses_devolver_abono').priceFormat({
        allowNegative: true,
        prefix: '$ ',
        centsSeparator: ',',
        thousandsSeparator: '.',
    ...
    //ACTUALIZAMOS EL VALOR DE LA OBLIGACION PARA GUARDARLO EN BASE
    $("#valor_obligacion_pp_abono").val(valorObligacion);
    $("#text_valor_obligacion_abono").text(valorObligacion);
    $('#text_valor_obligacion_abono').priceFormat({
        allowNegative: true,
        prefix: '$ ',
        centsSeparator: ',',
        thousandsSeparator: '.',
    ...
    if (abono > valorPresente){
        ...
        $("#gmf_ppa").val(gmf);
        $("#text_gmf_ppa").text(gmf);
        $('#text_gmf_ppa').priceFormat({
            allowNegative: true,
            prefix: '$ ',
            centsSeparator: ',',
            thousandsSeparator: '.',
    ...
    //INTERESES MORA
    $("#intereses_mora_abono").val(0);

}

function calcularReliquidacionPPAAnterior(){
    ...
    if ($("#fecha_pago_pactada").val() != "" && $("#fecha_real_pago").val() != "")
    {
        ...
        //NUEVO VALOR OBLIGACION
        var valorObligacion = valorObligacionPactada - interesesDevolver - abono;
        $("#nuevo_valor_obligacion").val(valorObligacion);
        $("#text_nuevo_valor_obligacion").text(valorObligacion);
        $('#text_nuevo_valor_obligacion').priceFormat({
            allowNegative: true,
...
function calcularReliquidacionPPAbonoAnterior(){
    ...
    //INTERESES A DEVOLVER
    var interesesDevolver = Math.round((valorPresente / potencia) - valorPresente);
    var interesesSumarObligacion = 0;
    if (diasValidacionIntereses <= 0){
    	interesesSumarObligacion = interesesDevolver;
    	interesesDevolver = 0;    	
    ...
    //NUEVO VALOR OOBLIGACION - VALOR PRESENTE
    var valorObligacionPresente = valorPresente - (interesesDevolver + parseFloat(abono)) + interesesSumarObligacion;
    if (valorObligacionPresente <= 0)
        valorObligacionPresente = 0;
    $("#nuevo_valor_obligacion_abono").val(valorObligacionPresente);
    $("#text_nuevo_valor_obligacion_abono").text(valorObligacionPresente);
    ...
    if (abono > valorPresente){
        ...
        //REMANENTES DISPONIBLES
        var remanentes = abono - (interesesDevolver + parseFloat(valorPresente));
        $("#text_remanentes_disponibles").text(remanentes);
        $('#text_remanentes_disponibles').priceFormat({
            allowNegative: true,
            prefix: '$ ',
...
function editReliquidacionAbono(idReliquidacionPP, idReliquidacion){
    ...
    $("#content_abonos").load('admindex.php', { Ajax:true, mod: 'reliquidaciones', action:'reliquidacionPPAbonos', idReliquidacionPP : idReliquidacionPP, idReliquidacion: idReliquidacion, id_operacion : idOperacion, tipo :'PPA'}, function () {
        closeNotify();
...
<?php
    ...
    while (!$rsDataReliquidacionPP->EOF){
        ...
        if ($esRegistroBase){
            $this->reliquidacionPPBase($idReliquidacionPP, $idOperacion, "PPA");
        }
        else{
                ...
                <table id="listReliquidaciones_<?=$idReliquidacionPP?>" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" style="width:100%;">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td align="right"><?=formato_moneda($rsDataReliquidacionPP->fields["intereses_devolver"])?></td>
                            ...
                            <td align="right">
                                <?php
                                    if ($rsDataReliquidacionPP->fields["abono"] > $rsDataReliquidacionPP->fields["valor_obligacion_pp"])
                                        ...
                                        //echo formato_moneda($rsDataReliquidacionPP->fields["valor_obligacion_pp"] - ($rsDataReliquidacionPP->fields["abono"] + $rsDataReliquidacionPP->fields["intereses_devolver"]));
                                        echo  formato_moneda($rsDataReliquidacionPP->fields["nuevo_valor_obligacion"]);
                                    }
    ...
    if (!$hayDatos)
        $this->reliquidacionPPBase(0, $idOperacion,"PPA");
?>
