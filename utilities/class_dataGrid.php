...
class DataGrid{
    ...
    function displayDataGrid() {
        ...
        if ($this->optionsHeader){
            $strDatos .= "<div class='row-fluid'><div class='content_menu_grilla'>";
    ...
    function getOptionsHeader() {
        ...
        if (is_array($this->arrOptionsHeader)){
            ...
            foreach($this->arrOptionsHeader as $arrOptions){
            	$label = $arrOptions["label"];
            	$link = $arrOptions[$label];
            	$classBtn = $arrOptions["classBtn"];
            	$classIcon = $arrOptions["classIcon"];

				if ($link != ""){                    	
					$optionsHeader .= '<a href="'.$link.'" class="btn '.$classBtn.' btn-sm '.$this->classOptionsHeader.'" style="margin-top:5px;"> <i class="fa '.$classIcon.' fa-lg btn-crud"></i> '.$label.'</a>&nbsp;';
				}
				else
					$optionsHeader .= $label;
            }
     ...
     */
    function addOptionsHeader($label,$link, $classBtn = "btn-primary", $classIcon="fa-plus-square") {

        $this->arrOptionsHeader[] = array($label=>$link, "label"=>$label,"classBtn"=>$classBtn, "classIcon"=>$classIcon);
    ...
    function getDataGrid() {
        ...
        if ($this->SQL){
            ...
            $lista = $this->getListData($this->connection);
                        ...
                        foreach($this->arrColLink as $arr){
                            ...
                            if ($arr[5]=="left"){
                                ...
                                if ($arr[4]=="popup")
                                    ...
                                    //&oacute; $dataGrid->addColLink("Ver Departamentos","<center><a href=\"javascript:{function};\"><img src='./images/editar.png' title='Editar Departamento' alt='Editar Departamento' border='0'/></a></center>","cargarDepartamentos",$arrVarGet1,"functionjs","right");
     ...
     * Funciòn para obtener el listado de titulos para el grid
     */
    function getListData($connection = "") {
        ...
        $lista = $connection->PageExecute($this->SQL.$this->WHERE.$this->ORDER_BY,$this->totalRegPag,$this->page, false, 0);
